...
- 支持即时发布（刷新后生效）
- 提供基础校验、前端容错、每日备份与回滚
- 支持跨平台素材目录准备（按权限生成 Windows/macOS/Linux 建目录命令，ZIP 作为备用）
...
- 支持多次选择多个素材目录后一次性导入
- 支持生成“素材目录结构”命令：Windows PowerShell 与 macOS/Linux 终端命令同时显示，自动按登录权限生成目录
- 保留 ZIP 目录模板导出作为备用；若浏览器下载卡在 `.crdownload`，请优先使用“生成建目录命令（推荐）”
...
4. 按顺序补充录入：素材绑定
   ...
   - 推荐在“新增学员成果”点击“生成建目录命令（推荐）”，复制对应系统命令后在教师电脑上生成目录结构
   - Windows 教师使用 `Windows PowerShell 命令`；macOS/Linux 教师使用 `macOS/Linux 终端命令`
   - 命令会在桌面生成 `素材目录结构_年月日时分` 总目录，并在其中生成各个 `成果ID_成果名称` 子目录
   - 每个子目录中的 `请将素材放在此目录.txt` 只是提示文件，不会被素材目录导入识别为素材
   - “导出素材目录结构”ZIP 仅作为备用；如浏览器下载卡住，可不用它
5. 点击“保存到草稿库”
6. 点击“保存并发布（推荐）”
...
用于批量准备成果素材文件夹，推荐使用“生成建目录命令（推荐）”，不依赖 ZIP 下载。

操作步骤：
1. 在收集器中进入“新增学员成果”
2. 点击“生成建目录命令（推荐）”
3. 根据教师电脑系统选择对应命令：
   - Windows：复制 `Windows PowerShell 命令`，打开 PowerShell，粘贴后回车
   - macOS/Linux：复制 `macOS/Linux 终端命令`，打开终端，粘贴后回车
4. 系统会在桌面生成 `素材目录结构_年月日时分` 总目录
5. 每个成果会生成一个 `成果ID_成果名称` 子目录
6. 教师把图片、视频、PDF 或互动网页文件放入对应成果目录
7. 回到“新增素材绑定”，选择该总目录并执行“一键导入素材目录”

说明：
- `请将素材放在此目录.txt` 是提示文件，不会被当作素材导入。
- “导出素材目录结构”ZIP 保留为备用；若浏览器下载卡在 `.crdownload`，请改用建目录命令。
- 命令按当前登录权限生成：教师只生成本人社团成果目录，超级管理员可生成全部成果目录。
