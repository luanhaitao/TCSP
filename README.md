...
- 教师登录：
  - 仅可查看/维护本人社团范围内数据
  - “新增学员成果”中的“所属社团”下拉仅显示本人社团
  - 学员成果导入、素材目录导入会自动校验范围并拦截越权记录
- 超级管理员登录：
  - 可查看/维护全量数据，不受社团范围限制

建议使用流程：
1. 点击“读取当前基础数据”
2. 社团信息可先“下载社团导入模板（CSV）”，批量填写后“一键导入社团（CSV/Excel）”
3. 学员成果可“下载成果导入模板（CSV）”，批量填写后“一键导入成果（CSV/Excel）”
4. 按顺序补充录入：素材绑定
   - 可直接“选择素材根目录（可多次追加多个目录） -> 开始一键导入素材目录”
...
素材目录导入命名规则：
- 目录名必须为：`成果ID_成果名称`（例如 `A001_智能红绿灯`）
- 支持文件类型：`jpg/png/gif/webp`、`mp4/mov/webm`、`pdf`
- 视频会优先尝试首帧/同名图片作为缩略图
